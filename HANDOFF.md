...
- [x] **Link article #3 into article #4** — done 2026-09-24 when #3 was written. The TODO comment at the top of `blog/signs-app-is-hiding-its-real-rate.html` is gone; its "mid-market rate" definition now links to the explainer, and article #1's mid-market bullet does too.
- [x] **Checked 2026-09-24 — articles #1 and #2 are still blocked.** The 2026-09-18 recheck (`e5223b3`) confirmed standard rates for Ria (62.278) and Sendwave (61.858) only. **Remitly** is still a promo at 64.53 (`verified: false`, well above mid-market) and **MoneyGram** is still `rate: null` — so the Wise-vs-Remitly (#1) and MoneyGram-vs-Ria-vs-Instarem (#2) head-to-heads have no trustworthy data to build on and stay on hold. Only Instarem, Ria, and Sendwave are currently ranked.

**Other open items surfaced 2026-09-18 (not done yet):**
- [ ] **Wise "flicker to promo" risk — needs a decision.** The updater's promo guardrail allows only a ₱0.01 gap above mid-market (`PROMO_TOLERANCE` in `update_rates.py`). In a test the live Wise feed (62.897) was above the free FX source (62.7776), which would have flagged Wise itself as a promo and unranked it. Probably safe at the 00:00 UTC run (both sources update near then), but fragile. Options: widen the tolerance slightly (also loosens promo detection) or compare against Wise's own rate. Watch `git log` for a day where Wise vanishes from the ranking. **Update 2026-09-24: this is now happening.** Wise is at 62.7676 against a 62.71 mid-market fallback — about 0.06 above — so it is flagged `verified: false` and is unranked, exactly the failure mode predicted. Worth deciding on soon, since it drops the provider the site's whole standard-rate story leans on.
...
- [ ] **Next article to write:** #5 (bank deposit vs. cash pickup vs. GCash) — it's the next one in the backlog with no data dependency. #1/#2 remain blocked on Remitly/MoneyGram (see above); #3 and #4 are published. Keep #9 (Christmas) on its early-November deadline.
- [ ] **Create a list of TikTok topic ideas for TrueRatePH** (added 2026-09-18). Note: an older TikTok script already exists unpublished in `content/` (dated 2026-07-26), and the YouTube/video plan was paused on 2026-09-16 in favor of SEO/blog — so decide up front whether TikTok is a light add-on (e.g. repurposing the blog articles as short clips) or a new channel push, given the 2-4 hrs/week budget.
...
- **10-article backlog planned 2026-09-16 — write in this exact order whenever "post an article" is asked, don't reshuffle without the user explicitly asking to:**
  ...
  3. [x] **PUBLISHED 2026-09-24** — "What Is the Mid-Market Rate, and Why It's the Only Number That Matters?" — [`blog/what-is-the-mid-market-rate.html`](blog/what-is-the-mid-market-rate.html). Foundational explainer; links to the live rate ticker on the homepage. ~1,260 words, generic (no provider names), all example numbers hypothetical so it won't go stale. **Plain-language rewrite 2026-09-24 at the user's request** ("make it easier to understand"): the user asked whether "mid-market rate" and "exchange rate" are the same and wanted the simpler term used throughout. They aren't the same — an app's own quote is an exchange rate too, so a blanket swap would have destroyed the article's central comparison ("compare the exchange rate to the exchange rate") and thrown away the SEO term that the title, slug, and articles #1/#4 all point at. **Decision: keep "mid-market rate" in the title/H1/slug/meta/JSON-LD, but say "the real exchange rate" in the body and "the app's rate" for provider numbers**, plus a new "Isn't that just the exchange rate?" section that answers the user's exact question (likely a real search query too). Also cut the finance jargon (interbank/spot rate, wholesale price, shell game, effective rate, drift), replaced the abstract bank buy/sell paragraph with a money-changer "We Buy / We Sell" board analogy, and rebuilt the worked example around pesos-per-dollar: App A "no fee" at 60.14 → ₱30,070, keeps ~₱930 (3%); App B with a $5 fee at 61.80 → ₱30,591, keeps ~₱409 (1.3%), so the app *with* the fee sends ₱521 more. App B's ₱30,591 is deliberately the same figure article #4 already uses. Deliberately scoped to *not* eat #6 (fees vs. markup) or #10 (timing — kept to one line). **The gap formula is fee-aware on purpose:** step one converts a quote into an *effective* rate (pesos received ÷ dollars paid) before measuring it against mid-market, so the article can't contradict #4's "don't judge the fee and the rate on their own" or the site's own after-fees ranking. **Known tension to revisit:** the article tells readers a gap of a few centavos above mid-market is just feed noise, which is looser than the real `PROMO_TOLERANCE` of ₱0.01 in `update_rates.py` — the "How we use it here" section papers over this by saying the check is "deliberately strict," but if the Wise tolerance decision above widens the guardrail, tighten that sentence to match. Also note: the article says the homepage figure is "refreshed daily," which is accurate (the `open.er-api.com` feed updates once per ~24h) — but `index.html` still labels that badge "Mid-market rate right now" and article #4 calls it "live." **Separate open decision: reword the homepage badge, or accept it.** Listed in `blog/index.html` and `sitemap.xml`; articles #1 and #4 now link into it. Verified in the local preview: renders correctly, all links resolve, JSON-LD and sitemap parse, no console errors. Optional: a personal anecdote could still go in the intro (HTML comment marks the spot).
  4. [x] **PUBLISHED 2026-09-18** — "7 Signs a Money Transfer App Is Hiding Its Real Rate From You" — [`blog/signs-app-is-hiding-its-real-rate.html`](blog/signs-app-is-hiding-its-real-rate.html). Direct sequel to article #1's angle, generic (no provider names), ~1,100 words, example numbers are clearly hypothetical so it won't go stale. Pulled ahead of #1-#3 because #1/#2 are blocked on fresh Remitly/MoneyGram rates (see section 6). Listed in `blog/index.html` and `sitemap.xml`. **Later:** once article #3 exists, link its "mid-market rate" definition to it (TODO comment at the top of the article file). Optional: a personal anecdote could still be added to the intro (marked with an HTML comment).
